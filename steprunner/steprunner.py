...
def handler(event, context):
    ...
    try:
        logger = general_functions.get_logger(survey, lambda_name,
                                              environment, run_id)
    except Exception as e:
        error_message = general_functions.handle_exception(e, lambda_name,
    ...
    if glue_spark_flag:
        ...
        logger.info('Error Steprunner :glue_job_name has not been set.Check glurunner lambda log for more information')
    ...
    try:
        ...
        logger.info(f"Running {lambda_name} Lambda to invoke Step Function {sfn_arn.split(':')[-1]}")

        # Expects a 'Payload' dictionary at the top-level in the event.
        if event.get('Payload') is None:
            raise KeyError("Top-level 'Payload' object attribute expected in event but not found")
    ...
    return {
        "statusCode": 200,
        "body": sfn_response['executionArn']
    }
